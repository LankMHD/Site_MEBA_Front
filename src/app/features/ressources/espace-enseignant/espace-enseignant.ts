...
@Component({
  ...
  styleUrls: ['./espace-enseignant.scss']
})
...
// Définir les catégories fixes directement dans le template ou dans le composant
readonly categories = [
  { id: 1, name: 'Préscolaire' },
  { id: 2, name: 'Primaire' },
  { id: 3, name: 'Primaire bilingue' },
  { id: 4, name: 'Post-Primaire' },
];
     ...
     this.apiService.getPublicDocuments(0, 20).subscribe({
       next: (response) => {
        ...
        const filteredDocs = response.data.content.filter(
          (doc: Document) => doc.typeDocument === 'DOCUMENT EDUCATION'
        );
   ...
   getCategoryLabel(category: string): string {
     const labels: Record<string, string> = { 'PRESCOLAIRE': 'Préscolaire', 'PRIMAIRE': 'Primaire', 'PRIMAIRE BILINGUE': 'Primaire bilingue', 'POST-PRIMAIRE': 'Post-Primaire', 'AUTRE': 'Autre' };
